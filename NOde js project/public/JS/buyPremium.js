...
document.getElementById("premium").onclick = async function (e) {
    ...
    // console.log(token)
    const response = await axios.get('http://3.215.181.196:4000/login/premium', { headers: { "Authorization": token } })
    ...
    var options = {
        ...
        "handler": async function (response) {
            ...
            await axios.post('http://3.215.181.196:4000/login/updatetransactionstatus', {
    ...
    rzrpay.on('payment.failed',async  (response) => {
        ...
        await axios.post('http://3.215.181.196:4000/login/updatetransactionstatus',{ status : "Failed"}, { headers: { "Authorization": token } }).then(res => {
                if (res.status === 402) {
